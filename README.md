...
| [ComfyUI-DNNodes](https://github.com/ljxdn/ComfyUI-DNNodes) | ljxdn | ![Conditioning][cat-cond] | Three asset-card nodes for the H3 director stage: **资产卡** (`H3MediaLoader` — image with paste/drop/dir-picker + MD5 dedupe, audio with waveform trim/mute, asset name + longest-edge cap, outputs `media/image/audio/prompt`), **资产卡 to Director Group** (`DNMediaToDirectorGroup` — packs cards into an `MMX_DIR_GROUP` for MiniMax H3 Director / Groups Combine) and **资产卡Group拆分** (`DNGroupSplit` — splits a group back into per-slot `image_1..9` / `video_1..3` / `audio_1..3` + `prompt` for the official per-slot workflow). Up to 9 connections on one `medias` port (connection order = `<Picture N>` numbering), per-link audio on/off, skipped-card highlighting, and a rich-text prompt editor. Needs [ComfyUI_MiniMaxH3_Director](https://github.com/AIMixer/ComfyUI_MiniMaxH3_Director); no extra pip deps. |
...
| [comfyui-AICG3D](https://github.com/JGRFW/comfyui-AICG3D) | JGRFW | ![Conditioning][cat-cond] | Unified H3 creation workbench merging nkxx188's Easy + goohai's H3_Integration (unofficial; node IDs unchanged, `Easy` display names → `Aicg` — old workflows import without rewiring): one-model loader with add/remove LoRA slots (up to 4, searchable with thumbnails/sizes/trigger hints, no external LoRA loader needed), click-to-`@` material references and a material library panel, unified skills + H3 scene presets directory (18 skill packs), a 144-template prompt library (~900k chars, 8 categories incl. 21 H3-specific) with a template node and panel, an AICG advanced sampler collapsing the 5-node sampling chain into one, and a bundled offline GGUF prompt-optimization engine (the license-less `comfyUI-llama-TE` code was removed upstream). GPL-3.0 (per-source licenses in `third_party/`). |
...
| [ComfyUI-FastH3Patcher](https://github.com/Nynxz/ComfyUI-FastH3Patcher) | Nynxz | ![Acceleration][cat-accel] | Run FastVideo's **FastH3** on top of the fl2va checkpoint you already have instead of downloading a second 44 GB model. Only two things actually differ from fl2va: the reparameterised adaLN timestep path (`adaln_proj.*` / `adaln_t_table`, **149 MB**) and the 50 `attn.to_gate_compress` layers that exist only in FastH3 (**3.85 GB**, read solely by VSA sparse attention — optional on dense / Sol-Attn / SLA runs). Both are grafted at load time, the gates via `add_object_patch` since a LoRA loader can never reach a module the base checkpoint doesn't declare. Patch files live in `models/model_patches/` — full 4.0 GB or adaLN-only 149 MB. No dependencies; ships an example T2V workflow. |
...
| [H3-Relight (bruxosdovfx)](https://github.com/NyckM/H3-Relight-Minimax-ComfyUI) | NyckM | ![Prompt][cat-prompt] | Lighting studio for MiniMax H3 — place up to three lights, set direction/intensity/color, background and atmosphere, and the node compiles a lighting-reference image + prompt for H3 Edit. Not a lighting adapter: it generates the reference and prompt (perfect illumination adherence not guaranteed). PT/EN docs; Apache-2.0. Pairs with Singularity (recommended model) and a 3-step turbo LoRA; dep list includes the author's h3 Frames + Compare nodes. |
